...
createTable PROC
    ...
    ;push rbx            ; Save non-volatile register rbx (used for duplicate check)
    ;push rdi            ; Save rdi (used for keyword address)
    ;push rsi            ; Save rsi (used for concatStr and table manipulation)
    ;push rdx            ; Save rdx (used for address calculations)
    ;sub rsp, 32         ; Allocate space for local variables, ensure stack alignment


    ;mov r9, rcx ; for testing
    ;mov r11, rdx; for testing
...
skip_character:
    ...
    ; RCX = pointer to the buffer
    ; RDX = buffer size (length)    
    ; Assume cipherText holds the encoded data that needs to be copied to the buffer
    ;lea rsi, table      ; Load the address of cipherText (the data to copy)
    ;mov rdi, r11           ; Move the buffer address (passed as first argument) into RDI
    ;mov rcx, 25          ; Move the buffer size (passed as second argument) into RCX
